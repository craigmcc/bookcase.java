...
import org.craigmcc.bookcase.service.BookService;
...
public class BookEndpoints {
    ...
    private static final Logger LOG = Logger.getLogger(BookEndpoints.class.getName());
    ...
    @APIResponses(value = {
            ...
            @APIResponse(
                    ...
                    description = "Missing book message.",
                    responseCode = "404"
            ),
            @APIResponse(
                    content = @Content(mediaType = MediaType.TEXT_PLAIN),
    ...
    @GET
    @Operation(description = "Find all books, ordered by title.")
    @APIResponses(value = {
            @APIResponse(
                    content = @Content(schema = @Schema(implementation = Book.class)),
    ...
    @GET
    @Path("/title/{title}")
    @Operation(description = "Find books matching title segment.")
    @APIResponses(value = {
            @APIResponse(
                    content = @Content(schema = @Schema(implementation = Book.class)),
                    description = "The found books.",
    ...
    public Response findByTitle(
            @Parameter(description = "Title matching segment of books to find.")
            @PathParam("title") String title
    ) {
        try {
            return Response.ok(bookService.findByTitle(title)).build();
        } catch (InternalServerError e) {
            LOG.log(SEVERE, e.getMessage(), e);
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                    .entity(e.getMessage())
    ...
    @POST
    @Operation(description = "Insert a new book.")
    @APIResponses(value = {
            @APIResponse(
                    content = @Content(schema = @Schema(implementation = Book.class)),
                    description = "The inserted book.",
                    responseCode = "201"
            ),
            @APIResponse(
                    content = @Content(mediaType = MediaType.TEXT_PLAIN),
    ...
    public Response insert(
            @Parameter(
                    description = "Book to be inserted.",
                    name = "book",
        ...
        try {
            ...
            URI uri = UriBuilder.fromResource(BookEndpoints.class)
